package mainGame;

import mapClasses.GameMap;
import org.newdawn.slick.*;
import org.newdawn.slick.Graphics;
import org.newdawn.slick.Image;
import org.newdawn.slick.geom.Rectangle;
import org.newdawn.slick.gui.TextField;
import org.newdawn.slick.state.BasicGameState;
import org.newdawn.slick.state.StateBasedGame;


/**
 * Created by Kingo on 30-10-2015.
 */
public class PlayingWindow extends BasicGameState
{
    Texture textures;
    int theWidth = Main.ScreenWidth;
    int theHeight = Main.ScreenHeight;
    String[] names = new String [] {"Fred", "George", "Ron", "Ginny"};
    GameObjects gameObjects;
    int[] currentResources = new int[5];
    int turn = 0;

    GameMap map;
    boolean buildMap = true;

    int sizeX = Main.ScreenWidth/4;
    int sizeY = Main.ScreenHeight/10;

    TextBoxBase chatBox;
    TextField chat;
    String chatText = "";
    Font font;

    @Override
    public void init(GameContainer gameContainer, StateBasedGame stateBasedGame) throws SlickException
    {
        gameObjects = new GameObjects();
        textures = new Texture();
        textures.initPlayingWindowTextures();
        map = new GameMap();

        font = new TrueTypeFont(new java.awt.Font("Verdana",
                java.awt.Font.PLAIN, 12), true);
        chatBox = new TextBoxBase();
        chat = new TextField(gameContainer, font, 5, Main.ScreenHeight-sizeY/2-5, Main.ScreenWidth/5, sizeY/2);
    }

    @Override
    public void update(GameContainer gameContainer, StateBasedGame stateBasedGame, int i) throws SlickException
    {
        map.update();

        if(gameContainer.getInput().isKeyPressed(Input.KEY_ENTER) && chat.getText()!="") {
            chatText = chat.getText();
            chatBox.newMessage(chatText, PlayerStats.name);
            chat.setText("");
        }
    }

    @Override
    public void render(GameContainer gc, StateBasedGame stateBG, Graphics g) throws SlickException {
        if (buildMap)
        {
            map.GameMapGameMap();
            buildMap = false;
        }
        map.render(g);
        gameObjects.PlayerList(35, 35, g, PlayerStats.names, turn);
        gameObjects.ResourceBar(200, 10, g, currentResources[0], currentResources[1], currentResources[2], currentResources[3], currentResources[4]);
        gameObjects.buildingCost.draw(theWidth - 205, theHeight - 255, 200, 250);
        gameObjects.TradePopupWindow(theWidth - 215, theHeight - 55, gameObjects.trade, PlayerStats.names[turn], g);
        gameObjects.IncomingTradeWindow(theWidth / 2 - 200, theHeight / 2 - 150, gameObjects.tradeWindow, PlayerStats.names[turn], g);
        gameObjects.OfferWindow(theWidth / 2 - 200, theHeight / 2 - 150, gameObjects.offerWindow, PlayerStats.names[turn], g);
        gameObjects.TradeWithWindow(Main.ScreenWidth-200, 200, g, names);

        g.setColor(Color.white);
        chatBox.render(g, gc);
        chat.render(gc, g);
    }

    @Override
    public int getID() {
        return 3;
    }

}
